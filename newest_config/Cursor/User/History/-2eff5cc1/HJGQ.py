...
class SCurveTool(BaseTool):
    ...
    def draw(self, painter):
        ...
        if self.drawing and self.start_point and self.end_point:
            ...
            if not self.preview_element:
                self.preview_element = SCurveElement(self.start_point, self.end_point, self.canvas.line_color)
            else:
    ...
    def _create_element(self):
        ...
        if self.start_point and self.end_point:
            element = SCurveElement(self.start_point, self.end_point, self.canvas.line_color)
            # Use default line thickness from DrawingElement
            element.line_thickness = 2  # Default value
            element.dash_pattern_key = "Solid"  # Default value
