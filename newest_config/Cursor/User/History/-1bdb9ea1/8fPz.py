...
class SCurveElement(DrawingElement):
    ...
    def _draw_shape(self, painter):
        ...
        dx = self.end_point.x() - self.start_point.x()
        dy = self.end_point.y() - self.start_point.y()
        base_angle = math.atan2(dy, dx) * 180 / math.pi
        
        # Calculate the perpendicular angle (90 degrees from base angle)
        perp_angle = base_angle + 90
        
        # Draw the first half-circle (centered at 1/4 of the distance)
        ...
        # Start angle is the perpendicular angle
        # Span angle is 180 degrees
        painter.drawArc(rect1, int(perp_angle * 16), 180 * 16)
        
        # Draw the second half-circle (centered at 3/4 of the distance)
        # Calculate the rectangle for the second half-circle
        rect2 = QRectF(
            center2.x() - radius, 
            center2.y() - radius, 
            radius * 2, 
        ...
        # Start angle is the perpendicular angle plus 180 degrees
        # Span angle is 180 degrees
        painter.drawArc(rect2, int((perp_angle + 180) * 16), 180 * 16)
