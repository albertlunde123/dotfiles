...
class DistanceBasedCalculator(EnergyCalculator):
    ...
    def _energy_contribution(self, distance: float, atom1_symbol: str, atom2_symbol: str) -> float:
        ...
        # Get VdW radii for the atoms
        vw1 = vdw_radii[atom1_symbol]
        vw2 = vdw_radii[atom2_symbol]

        # If the distance is less than the sum of the VdW radii, the atoms repel each other
        if distance < vw1 + vw2:
            return 100
        
        # If the distance is greater than the sum of the VdW radii, the atoms attract each other
        else:
            return 1/distance**2
