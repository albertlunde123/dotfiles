...
class SelectMoveTool(BaseTool):
    ...
    def mouseMoveEvent(self, event):
        ...
        if self.current_mode == self.MODE_MOVE and self.drag_started and self.last_mouse_pos:
            ...
            # Find Best Snap Adjustment using get_snap_target
            best_adjustment = QPointF(0, 0)
            # Use a very large initial value for min_snap_dist_sq
            ...
            for element in selected_elements:
                if hasattr(element, 'snap_points') and element.snap_points:
                    for point in element.snap_points:
                        # Calculate potential position after raw movement
                        potential_pos = point + QPointF(dx_raw, dy_raw)

                        # Find potential snap target for this point
                        snapped_pos, snap_type = get_snap_target(
                            potential_pos,
                            non_selected_elements,
                            self.canvas.grid_manager,
                            self.snap_threshold
                            # Removed is_placement_mode=False to use default snap strength
                        )

                        if snap_type:
                            # Calculate the adjustment needed to reach the snap target
                            adjustment = snapped_pos - potential_pos
                            adjustment_dist_sq = adjustment.x()**2 + adjustment.y()**2

                            # Check if this adjustment is smaller than the current best
                            # Also ensure the adjustment is within the snap threshold distance
                            if adjustment_dist_sq < min_snap_dist_sq and adjustment_dist_sq < self.snap_threshold ** 2:
                                ...
                                # Store the actual snap target for visual feedback
                                self.active_snap_target = snapped_pos


            # Apply the movement with the single best snap adjustment found
