...
class SelectMoveTool(BaseTool):
    ...
    def __init__(self):
        ...
        # Snap activation threshold
        self.snap_activation_threshold_sq = 9 # 3 pixels squared
    ...
    def mouseMoveEvent(self, event):
        ...
        if self.current_mode == self.MODE_MOVE and self.drag_started and self.last_mouse_pos:
            ...
            # Determine if snapping should be considered based on movement threshold
            raw_move_dist_sq = dx_raw**2 + dy_raw**2
            can_apply_snap = raw_move_dist_sq > self.snap_activation_threshold_sq
                    ...
                    for point in element.snap_points:
                        ...
                        if snap_type:
                            ...
                            if adjustment_dist_sq < min_snap_dist_sq and adjustment_dist_sq < self.snap_threshold ** 2:
                                ...
                                # Store the potential best adjustment and target
                                best_adjustment = adjustment
                                current_active_snap_target = snapped_pos

            # Update the persistent snap indicator target (show potential snap even if not applied)
            self.active_snap_target = current_active_snap_target

            # Apply the movement - only add adjustment if snapping is allowed
            final_dx = dx_raw
            final_dy = dy_raw
            if can_apply_snap:
