...
class SelectMoveTool(BaseTool):
    ...
    def __init__(self):
        ...
        self.snap_release_threshold_sq = 25 # 5 pixels squared
    ...
    def mouseMoveEvent(self, event):
        ...
        # Check if we need to release the snap lock
        ...
        if self.current_mode == self.MODE_MOVE and self.drag_started and self.last_mouse_pos:
            ...
            # Determine if snapping is currently allowed
            can_apply_snap = not self.recently_snapped

            # Optimization: Only check for snaps if raw movement is significant
            # or if snapping is not currently locked
            perform_snap_check = can_apply_snap or (abs(dx_raw) > 0.1 or abs(dy_raw) > 0.1)

            best_adjustment = QPointF(0, 0)
            current_active_snap_target = None # Temporary holder for this event's target

            if perform_snap_check:
                        ...
                        for point in element.snap_points:
                            ...
                            if snap_type:
                                ...
                                if adjustment_dist_sq < min_snap_dist_sq and adjustment_dist_sq < self.snap_threshold ** 2:
                                    # Only apply if snapping is allowed
                                    if can_apply_snap:
                                        min_snap_dist_sq = adjustment_dist_sq
                                        best_adjustment = adjustment
                                        current_active_snap_target = snapped_pos
                                    elif not current_active_snap_target: # Still show indicator even if locked
                                        current_active_snap_target = snapped_pos

            # Update the persistent snap indicator target
            self.active_snap_target = current_active_snap_target

            # Apply the movement
            final_dx = dx_raw + best_adjustment.x()
            final_dy = dy_raw + best_adjustment.y()

            # Check if a snap was actually applied in this step
            snap_applied_this_step = best_adjustment != QPointF(0, 0)

            for element in selected_elements:
                element.move_by(final_dx, final_dy)

            # Update snap lock state *after* movement
            if snap_applied_this_step and not self.recently_snapped:
                self.recently_snapped = True
                self.pos_at_last_snap = current_mouse_pos # Store position *before* snap adjustment?
                # print("Snap locked") # Debugging
