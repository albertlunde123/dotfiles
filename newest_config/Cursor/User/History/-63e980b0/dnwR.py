...
class SelectMoveTool(BaseTool):
    ...
    def __init__(self):
        ...
        self.last_snap_point = None   # Last point we snapped to
        self.total_movement = QPointF(0, 0)  # Total movement since drag started
    ...
    def _reset_state(self):
        ...
        self.total_movement = QPointF(0, 0)
    ...
    def mouseMoveEvent(self, event):
        ...
        if self.initial_drag_pos:
            self.total_movement = current_mouse_pos - self.initial_drag_pos
        
        # Check if we've exceeded drag threshold
        if not self.drag_started and self.start_pos:
            delta = current_mouse_pos - self.start_pos
        ...
        if self.current_mode == self.MODE_MOVE and self.drag_started and self.last_mouse_pos:
            ...
            if abs(dx_raw) < 0.1 and abs(dy_raw) < 0.1:
                if self.active_snap_target:
                    self.active_snap_target = None
                    self.canvas.update()
                return

            # Find Best Snap Adjustment
            best_adjustment = QPointF(0, 0)
            min_snap_dist_sq = self.snap_threshold ** 2
            self.active_snap_target = None
            non_selected_elements = [e for e in self.canvas.elements if not e.selected]

            for element in self.canvas.get_selected_elements():
                # Get the current position of the element
                current_pos = QPointF(element.rect.center())
                
                # Find potential snap point for this element
                snapped_pos, snap_type = self._find_best_snap_point(current_pos)
                
                if snap_type:
                    # Calculate the adjustment needed to snap
                    adjustment = QPointF(
                        snapped_pos.x() - current_pos.x(),
                        snapped_pos.y() - current_pos.y()
                    )
                    
                    # Update best adjustment if this one is better
                    adjustment_dist_sq = adjustment.x()**2 + adjustment.y()**2
                    if adjustment_dist_sq < min_snap_dist_sq:
                        min_snap_dist_sq = adjustment_dist_sq
                        best_adjustment = adjustment
                        self.active_snap_target = snapped_pos
                        self.last_snap_point = snapped_pos

            # Apply the movement with snap adjustment
            for element in self.canvas.get_selected_elements():
                element.move_by(dx_raw + best_adjustment.x(), dy_raw + best_adjustment.y())
    ...
    def _find_best_snap_point(self, original_pos: QPointF):
        ...
        grid_element_snap, ge_snap_type = get_snap_target(
            original_pos, self.canvas.elements, self.canvas.grid_manager, 
            self.snap_threshold, is_placement_mode=False
        )
        if ge_snap_type:  # Found a grid or element snap
            dist = dist_sq(original_pos, grid_element_snap)
            if dist < min_snap_dist_sq:
                # Check if we're moving away from the last snap point
                if self.last_snap_point and self.total_movement:
                    to_last_snap = QPointF(
                        self.last_snap_point.x() - self.initial_drag_pos.x(),
                        self.last_snap_point.y() - self.initial_drag_pos.y()
                    )
                    # If we're moving away from the last snap point, reduce snap strength
                    if (to_last_snap.x() * self.total_movement.x() < 0 or 
                        to_last_snap.y() * self.total_movement.y() < 0):
                        dist *= 2  # Make it harder to snap when moving away
