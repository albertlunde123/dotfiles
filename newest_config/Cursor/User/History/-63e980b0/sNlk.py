...
class SelectMoveTool(BaseTool):
    ...
    def mouseMoveEvent(self, event):
        ...
        if self.current_mode == self.MODE_MOVE and self.drag_started and self.last_mouse_pos:
            ...
            for element in self.canvas.get_selected_elements():
                if not hasattr(element, 'snap_points'): continue

                for source_point in element.snap_points:
                    potential_pos = source_point + QPointF(dx_raw, dy_raw)

                    # Check Grid Snap
                    grid_target = self.canvas.grid_manager.snap_to_grid(potential_pos)
                    if grid_target != potential_pos:
                        d_sq = dist_sq(potential_pos, grid_target)
                        if d_sq < (self.canvas.grid_manager.spacing / 8)**2 and d_sq < min_snap_dist_sq:
                            min_snap_dist_sq = d_sq
                            best_adjustment = grid_target - potential_pos
                            self.active_snap_target = grid_target

                    # Check Element Snap
                    element_target = find_element_snap_point(
                        non_selected_elements, potential_pos, self.snap_threshold
                    )
                    if element_target:
                        d_sq = dist_sq(potential_pos, element_target)
                        if d_sq < min_snap_dist_sq:
                            min_snap_dist_sq = d_sq
                            best_adjustment = element_target - potential_pos
                            self.active_snap_target = element_target

            # Apply movement with snap adjustment
            dx = dx_raw + best_adjustment.x()
            dy = dy_raw + best_adjustment.y()
            for element in self.canvas.get_selected_elements():
                element.move(dx, dy)
