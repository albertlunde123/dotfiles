...
import numpy as np # Needed for processing radii/sizes
...
def plot_cluster(cluster: Cluster,
                   dim: Literal[2, 3] = 3,
                   show_labels: bool = False,
                   use_radii_for_size: bool = True,
                   size_scale_factor: float = 100.0, # Adjust as needed
                   ax: Union[plt.Axes, Axes3D, None] = None):
    """Plots the atoms of a cluster in a 2D or 3D scatter plot.

    Atom marker sizes can be scaled by their Van der Waals radii.
    ...
    Args:
        ...
        use_radii_for_size: If True, scale marker size by VdW radius. Defaults to True.
        size_scale_factor: Multiplier for radii^2 to get marker size. Defaults to 100.
    ...
    if use_radii_for_size:
        # Scale size by radius squared (area)
        radii = np.array([atom.vdw_radius for atom in cluster.atoms])
        sizes = (radii**2) * size_scale_factor
    else:
        sizes = None # Use default scatter size

    if dim == 3:
        zs = [atom.position[2] for atom in cluster.atoms]
        ax.scatter(xs, ys, zs, s=sizes, marker='o') # Added s=sizes
        if show_labels:
        ...
        ax.scatter(xs, ys, s=sizes, marker='o') # Added s=sizes
        if show_labels:
