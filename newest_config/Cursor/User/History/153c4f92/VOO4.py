...
class PolylineElement(DrawingElement):
    ...
    def __init__(self, points, color):
        ...
        # Ensure points is a list and convert to QPointF
        if not points:
            points = [QPointF(0, 0), QPointF(10, 10)]  # Default points if empty
        self.points = [QPointF(p) for p in points]
        self.snap_points = self.points.copy()  # Create a copy to avoid reference issues
    ...
    def contains_point(self, point: QPointF) -> bool:
        """Check if the point is within a certain distance of any line segment in the polyline."""
    ...
    def move(self, dx: float, dy: float):
        """Move all points in the polyline by the specified delta."""
        delta = QPointF(dx, dy)
