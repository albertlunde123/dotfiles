...
from pyqt_drawing_app.utils.latex_renderer import LatexRenderer
...
class LatexTextTool(BaseTool):
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton:
            ...
            latex_code, ok = QInputDialog.getText(
                ...
                "LaTeX Text",
                "Enter LaTeX code:",
                text=self.latex_code if self.latex_code else ""
            )
    ...
    def draw(self, painter):
        ...
        if self.drawing and self.position and self.latex_code:
            ...
            if not self.preview_element:
                self.preview_element = LatexTextElement(
                    self.position,
                    self.latex_code,
                    self.renderer,
    ...
    def _create_element(self):
        """Create the LaTeX text element and add it to the canvas."""
...
""" % (self._color_to_rgb_values(), latex_code) # Use %s formatting for color
    ...
    def _color_to_rgb_values(self):
        ...
        if isinstance(self.canvas.current_color, QColor):
            r = self.canvas.current_color.red()
            g = self.canvas.current_color.green()
            b = self.canvas.current_color.blue()
            return f"{r},{g},{b}"
        else:
