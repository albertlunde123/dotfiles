...
object TypeChecker {
  ...
  def typeCheck(e: Exp, tenv: TypeEnv): Type = e match {
    ...
    case BlockExp(vals, vars, defs, exp) =>
      var tenv1 = tenv
      for (d <- vals) {
    ...
    case AssignmentExp(x, exp) =>
      ...
      checkTypesEqual(typeCheck(exp, tenv), Some(xtype), e)
      unitType
    case WhileExp(cond, body) =>
      val condType = typeCheck(cond, tenv) match {
        case BoolType() =>
        case _ => throw TypeError("condition in while loop should be boolean", e)
      }
      val bodyType = typeCheck(body, tenv)
      unitType
  }
