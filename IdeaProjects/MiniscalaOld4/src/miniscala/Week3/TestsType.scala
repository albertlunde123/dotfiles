...
object Test38 {
  ...
  def test(prg: String, out: Type, env: VarTypeEnv = Map[Var, Type](), fenv: FunTypeEnv = Map[Fun, (List[Type], Type)]()): Unit = {
    assert(typeCheck(parse(prg), env, fenv) == out)
  }

  def testFail(prg: String): Unit = {
    try {
      typeCheck(parse(prg), Map[Var, Type](), Map[Fun, (List[Type], Type)]())
      assert(false)
    } catch {
