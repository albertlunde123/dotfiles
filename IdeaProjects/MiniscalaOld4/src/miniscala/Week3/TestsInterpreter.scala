...
object Test37 {
  ...
  def test(prg: String, out: Val, env: VarEnv = Map[Var, Val](), fenv: FunEnv = Map[Fun, Closure]()): Unit = {
    assert(eval(parse(prg), env, fenv) == out)
  }

  def testFail(prg: String): Unit = {
    try {
      eval(parse(prg), Map[Var, Val](), Map[Fun, Closure]())
      assert(false)
    } catch {
