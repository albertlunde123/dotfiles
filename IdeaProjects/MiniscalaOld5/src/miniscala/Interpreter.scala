...
object Interpreter {
  ...
  def eval(e: Exp, env: Env): Val = e match {
    ...
    case b @ BlockExp(vals, defs, exp) =>
      ...
      for (d <- vals ++ defs)
         env1 = eval(d, env1, b)
      // we only loop over the vals now, as we want access to all defs when declaring the functions
     // var closure = Map[Id, Val]()
      //for (d <- defs) {
        //closure = closure + (d.fun -> ClosureVal(d.params, d.optrestype, d.body, env1, defs))
      //}
      //env1 = env1 ++ closure
      eval(exp, env1)
    case TupleExp(exps) =>
    ...
    case CallExp(funexp, args) =>
      ...
      val closure = eval(funexp, env) match {
        case ClosureVal(params, optrestype, body, env, defs) => ClosureVal(params, optrestype, body, env, defs)
  ...
  def eval(d: Decl, env: Env, b: BlockExp): Env = d match {
    case ValDecl(x, opttype, exp) =>
      checkValueType(eval(exp, env), opttype, d)
      env + (x -> eval(exp, env))
    case DefDecl(fun, params, optrestype, body) =>
      val closure = ClosureVal(params, optrestype, body, env, b.defs)
      env + (fun -> closure)
  }
