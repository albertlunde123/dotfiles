...
plotting_obj.optimizer.sigma = plotting_obj.optimizer.sigma * 0.5
plotting_obj.optimizer.sigma[370:] = plotting_obj.optimizer.sigma[370:] * 0.7
plotting_obj.plot_gaussian(alpha = 0.2, border = False)
plot_EI_gaussian(np.argmax(EI), plotting_obj)
plot_EI_gaussian(650, plotting_obj, 1, 1.39, 0.5)

plotting_obj.disable_borders()

# plot a gauss in the y axis, located at the EI_max

# ys = np.linspace(EI_center - 4*EI_sigma, EI_center + 4*EI_sigma, 100)

plotting_obj.ax_arr.plot(x, [np.min(optimize_obj.values)]*len(x), color = 'orange', linewidth = 1, linestyle = '-')
# plotting_obj.ax_arr.plot(gauss(ys), ys, color = 'purple', linewidth = 1)
# ys_2 = np.linspace(EI_center - 4 * EI_sigma, 0.143, 1000)
# x1 = x[np.argmax(EI)] * np.ones(len(ys_2))
# x2 = gauss(ys_2)
# plotting_obj.ax_arr.fill_betweenx(ys_2, x1, x2, color = 'purple', alpha = 0.5, edgecolor = 'black', linewidth = 0)
...
plotting_obj.save("../Rapport/figures/EI/EI_explainer.png")
